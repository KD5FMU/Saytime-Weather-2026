...
install_weather_scripts() {
    ...
    if [ -f "$SBIN_DIR/say24time.pl" ]; then
        ...
        log "Installed ASL3-compatible say24time.pl"
    fi
}
...
add_announcement_cron_if_needed() {
    ...
    if [ -z "$LOCATION_ARG" ] || [ -z "$NODE_ARG" ]; then
        ...
        log "24-hour entry example: 0 * * * * /usr/bin/perl /usr/local/sbin/say24time.pl ZIP_OR_LOCATION NODE_NUMBER >/dev/null 2>&1"
        return 0
    fi
