...
# v4: includes say24time.pl support for ASL3.
...
detect_existing_values_from_cron() {
    ...
    detected="$(echo "$cron_text" | awk '/say(time|24time)\.pl/ { for (i=1; i<=NF; i++) { if ($i ~ /say(time|24time)\.pl$/) { if ((i+2)<=NF) { gsub(/[^A-Za-z0-9.,_-]/, "", $(i+1)); gsub(/[^0-9]/, "", $(i+2)); print $(i+1) " " $(i+2); exit } } } }')"
    if [ -n "$detected" ]; then
...
install_or_preserve_say24time() {
    if [ -f "$SBIN_DIR/say24time.pl" ]; then
        log "Existing $SBIN_DIR/say24time.pl found; preserving it."
    else
        log "Installing ASL3-compatible say24time.pl"
...
any_saytime_cron_exists() {
    crontab -l 2>/dev/null | grep -Eq "say(time|24time)\.pl"
}

add_announcement_cron_if_needed() {
    if any_saytime_cron_exists; then
        log "A saytime.pl or say24time.pl crontab entry already exists; leaving it unchanged."
        return 0
    fi
    if [ -z "$LOCATION_ARG" ] || [ -z "$NODE_ARG" ]; then
        log "No existing saytime/say24time cron entry found."
