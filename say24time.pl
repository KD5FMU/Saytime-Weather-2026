...
# EX: /etc/asterisk/local/saytime_header.gsm,  /etc/asterisk/local/saytime_header.ul
...
if ($error == 1) {
  print "\nUsage: saytime.pl [<wx_flag>] nodenumber\n";
  exit;
}
...
if ($wx eq "YES") {
  ...
  if (-e "$outdir/condition.gsm") {
    ...
    my $content = do { local (@ARGV, $/) = "$outdir/condition.gsm"; <> };
    chomp $content;
    $content=~ s/\n//g;
    $FNAME = $FNAME . "$content" . " ";
}
