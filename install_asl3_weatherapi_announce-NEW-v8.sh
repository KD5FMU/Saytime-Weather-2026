...
# v8: downloads say24time.pl directly from KD5FMU/Saytime-Weather-2026 and patches it for ASL3.

set -u

REPO_TARBALL_URL="https://github.com/KD5FMU/Saytime-Weather-2026/raw/main/asl3-weatherapi-announce.tar.gz"
SAY24TIME_URL="https://raw.githubusercontent.com/KD5FMU/Saytime-Weather-2026/refs/heads/main/say24time.pl"
...
install_say24time_from_github() {
    local tmp="/tmp/say24time.pl.$$"

    log "Downloading say24time.pl from KD5FMU/Saytime-Weather-2026"
    curl -fL "$SAY24TIME_URL" -o "$tmp" || die "Could not download say24time.pl from $SAY24TIME_URL"

    # Patch HamVoIP/older AllStar sound path to ASL3 custom sound path.
    sed -i 's#my \$base = "/var/lib/asterisk/sounds";#my $base = "/usr/local/share/asterisk/sounds/custom";#' "$tmp"
    sed -i 's#/var/lib/asterisk/sounds#/usr/local/share/asterisk/sounds/custom#g' "$tmp"

    # Make the usage line match the installed file name if present.
    sed -i 's/Usage: saytime.pl/Usage: say24time.pl/g' "$tmp"

    # Basic sanity checks.
    grep -q 'my $base = "/usr/local/share/asterisk/sounds/custom";' "$tmp" || die "say24time.pl ASL3 sound path patch failed."
    grep -q '/usr/sbin/asterisk -rx' "$tmp" || die "Downloaded say24time.pl does not appear to contain Asterisk localplay logic."

    log "Installing ASL3-patched say24time.pl to $SBIN_DIR/say24time.pl"
    backup_file "$SBIN_DIR/say24time.pl"
    install -m 0755 "$tmp" "$SBIN_DIR/say24time.pl"
    rm -f "$tmp"

    [ -x "$SBIN_DIR/say24time.pl" ] || die "Failed to install executable $SBIN_DIR/say24time.pl"
}
...
show_config_notice() {
    ...
    echo
    echo " Confirm say24time.pl installed and patched:"
    echo
    echo "   ls -l /usr/local/sbin/say24time.pl"
    echo "   grep 'my \$base' /usr/local/sbin/say24time.pl"
    echo
    echo "======================================================================"
    echo
}
